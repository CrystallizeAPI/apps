...
import type { CrystallizeCustomer, CrystallizeOrder, CrystallizeProduct } from '~/graph/types';

function getPaymentStatus(order: CrystallizeOrder): PaymentStatus | null {
    if (Array.isArray(order.payment)) {
        const [payment] = order.payment;
        if (payment) {
            const status = payment.properties?.find((p) => p.property === 'Payment status')?.value;
            if (status) {
    ...
    if (!state.index) {
        ...
        if (state.customerStatus !== CustomersAll) {
            if (state.customerStatus === CustomersWithoutIncompleteData) {
                customersToSearchIn = customersToSearchIn.filter((c) => c.invalidLabels.length > 0);
            } else if ([CustomersWithOrders, CustomersWithoutOrders].includes(state.customerStatus)) {
                ...
                return Boolean(
                    customer.orders?.some((order) =>
                        order.cart.some((cartItem) => {
                            const product = state.products.find((p) => p.variants.some((v) => v.sku === cartItem.sku));
        ...
        state.customers.forEach((customer) => {
            customer.orders?.forEach((order) => {
                const paymentStatus = getPaymentStatus(order);
                if (paymentStatus && !state.allPossiblePaymentStatuses.some((p) => p.value === paymentStatus.value)) {
        ...
        searchEngineResult.forEach((resultSet) => {
            ...
            if (!state.selectedSubset || state.selectedSubset === resultSet.field) {
                resultSet.result.forEach((customerIdentifier) => {
                    const customer = state.customers?.find((c) => c.identifier === customerIdentifier);

                    if (customer && !result.customers.some((c) => c.identifier === customer.identifier)) {
...
export function searchIndexReducer(state: SearchState, action: SearchAction) {
    switch (action.type) {
        case 'set-payment-status': {
            state.paymentStatus = action.paymentStatus === PaymentStatusAll ? undefined : action.paymentStatus;
        ...
        case 'set-customer-status': {
            state.customerStatus = action.customerStatus as SearchState['customerStatus'];
            state.index = null;
            break;
        }
        case 'select-subset': {
            state.selectedSubset = action.subsetField === 'all' ? undefined : action.subsetField;
            break;
        }
