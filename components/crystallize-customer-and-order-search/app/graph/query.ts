...
export const getAllOrdersForTenant = async (tenantId: string): Promise<CrystallizeOrder[]> => {
    try {
...
function includeCustomerLabels(customer: CrystallizeCustomer): CrystallizeCustomer {
...
export const getAllCustomersForTenant = async (tenantId: string): Promise<CrystallizeCustomer[]> => {
    try {
        const res = await getClient().request(customerPageQuery, { tenantId });
        return res.customer.getMany?.edges?.map((e: CrystallizeCustomerSearchNode) => includeCustomerLabels(e.node));
    } catch (e) {
        console.log(e);
    }
...
export const getProductsFromOrders = async (
    ...
    orders.forEach((order) => (order.cart || []).forEach((cartItem) => skus.add(cartItem.sku)));
    ...
    try {
        ...
        res.product.getVariants?.forEach((e: any) => products.set(e.product.id, e.product));
    } catch (e) {
        console.log(e);
    }
