...
import type { CrystallizeCustomer, CrystallizeCustomerLabel, CrystallizeOrderLabel } from '~/graph/types';
...
function invalidLabelDisplay(label: CrystallizeCustomerLabel | CrystallizeOrderLabel) {
    switch (label) {
...
export default function Item({ customer }: { customer: CrystallizeCustomer }) {
    const deliveryAddress = customer.addresses?.find((address) => address.type === 'delivery');

    const allLabels: (CrystallizeCustomerLabel | CrystallizeOrderLabel)[] = [];
    allLabels.push(...customer.invalidLabels);

    return (
        <details key={customer.identifier} className="mb-2 details text-blac cursor-pointer">
            <summary className="flex flex-row relative bg-white shadow-sm rounded-md pl-10 pr-6 py-2 ">
                <div className="w-[20px] absolute left-[15px] arrow flex items-center justify-center">&#8595;</div>

                <div className="text-black flex flex-col w-4/12">
                    <span className="font-medium text-md">
                        {customer.firstName} {customer.lastName}
                    </span>
                    <span className="font-medium text-sm">{customer.orders?.length} order(s)</span>
                </div>
                        ...
                        <div className="w-full">
                            ...
                            {order.cart.map((cartItem, index) => (
                                <div key={index} className="flex items-center">
                                    <img src={cartItem.imageUrl} alt="" className="w-[50px] block mr-2" />
                                    {cartItem.name}
                                </div>
                            ))}
