...
import { getAllOrdersForTenant, getAllCustomersForTenant, getTenantId, getProductsFromOrders } from './../graph/query';

import { FieldRecord, getDefaultSearchState, searchIndexReducer } from '~/search-index';
...
export const loader: LoaderFunction = async ({ request }) => {
    ...
    return json({
        ...
        customers: customers.map((customer) => ({
            ...customer,
            orders: orders.filter((order) => order.customer?.identifier == customer.identifier),
        })),
        products,
    });
};
...
export default function Index() {
    ...
    const [state, dispatch] = useImmerReducer(searchIndexReducer, getDefaultSearchState({ customers, products }));
    ...
    const debouncedQueryChange = useMemo(
        () =>
            debounce(
                (e: React.ChangeEvent<HTMLInputElement>) => dispatch({ type: 'set-query', query: e.target.value }),
                300,
            ),
        [dispatch],
    );
    ...
    return (
        <div className="mx-auto max-w-[1200px] py-10 px-6">
            <h1 className="text-xl text-black font-bold mb-2">Customer & Order search</h1>
